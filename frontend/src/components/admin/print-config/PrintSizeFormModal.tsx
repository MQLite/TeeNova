...
function buildValues(target: PrintSize | null): FormValues {
  return {
    name:      target?.name      ?? '',
    code:      target?.code      ?? '',
...
export function PrintSizeFormModal({ open, onClose, editTarget, onSaved }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const input: CreateUpdatePrintSizeInput = { name, code, basePrice, isActive: values.isActive, sortOrder }

    setSaving(true)
    try {
      if (editTarget) {
        await printConfigApi.updateSize(editTarget.id, input)
      } else {
        await printConfigApi.createSize(input)
      }
      onSaved()
      onClose()
    } catch (err: unknown) {
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex items-center justify-end gap-2 pt-1">
          ...
          <Button type="submit" variant="black" size="sm" loading={saving}>
            {editTarget ? 'Save changes' : 'Add print size'}
          </Button>
        </div>
      </form>
    </Modal>
