...
/// <summary>
/// FROZEN VERBATIM COPY of <c>OrderProductionPdfService</c> as it stood immediately BEFORE the Jira
/// 10103 shared-helper extraction — including its own private <c>SplitVariantLabel</c> and
/// <c>SizeRank</c> implementations. Test-only: it lets a single test run render the same order through
/// the pre-extraction and post-extraction code paths within the same minute and assert EXACT BYTE
/// EQUALITY (the sheet's only nondeterminism is its minute-granular "Generated ..." timestamp).
///
/// Nothing in production references this file, and it must NOT be updated to track the later PDF
/// redesigns (Jira 10104/10105) — it is a historical baseline of the pre-10103 output.
/// </summary>
public static class LegacyOrderProductionPdfBaseline
{
    private static readonly CultureInfo NzCulture = CultureInfo.GetCultureInfo("en-NZ");

    static LegacyOrderProductionPdfBaseline()
    {
        QuestPDF.Settings.License = LicenseType.Community;
    }

    public static byte[] Generate(OrderDto order) => BuildDocument(order).GeneratePdf();

    // ── Document composition ────────────────────────────────────────────────────

    private static Document BuildDocument(OrderDto order)
    {
    ...
    private static void ComposeHeader(IContainer container, OrderDto order, string generatedAt)
    {
    ...
    private static void ComposeCustomerAndDelivery(IContainer container, OrderDto order)
    {
    ...
    private static void ComposeOrderSummary(IContainer container, OrderDto order)
    {
    ...
    private static void ComposeItems(IContainer container, OrderDto order)
    {
    ...
    private static void ComposeDesignOnlyItems(ColumnDescriptor col, OrderDto order)
    {
    ...
    /// </summary>
    private static void ComposeBannerItems(ColumnDescriptor col, OrderDto order)
    {
    ...
    private static void ComposeNotes(IContainer container, OrderDto order)
    {
